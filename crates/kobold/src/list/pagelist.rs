...
impl<T> Page<T> {
    fn new(cap: usize) -> NonNull<Self> {
        debug_assert_eq!(std::mem::size_of::<(NonNull<Head<T>>, usize)>(), std::mem::size_of::<NonNull<Page<T>>>());
...
struct Cursor<'a, T> {
    fold: usize,
    page: NonNull<Page<T>>,
    _pl: PhantomData<&'a mut PageList<T>>
}
...
impl<'a, T> Cursor<'a, T> {
    ...
    // pub fn reserve(self, cap: usize) -> ReserveIter<'a, T> {
    //     while let Some(next) = Page::as_mut(self.page).next {
    //         self.page = next;
    //     }
    // }
}
...
impl<'a, T> Iterator for Cursor<'a, T> {
    ...
    fn next(&mut self) -> Option<&'a mut T> {
        match Page::as_mut(self.page).get_mut(self.fold) {
            Some(item) => {
                self.fold += 1;
                Some(item)
            }
            None => {
                self.page = Page::as_mut(self.page).next?;
                self.fold = 1;

                Page::as_mut(self.page).get_mut(0)
            }
        }
    }
}
...
mod tests {
    ...
    fn page_list() {
        ...
        assert_eq!(&list.cursor().map(|i| *i).collect::<Vec<_>>()[..], &[42, 100, 404]);
    }
